...
export function useStageRunner() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      elapsedRef.current += 100 * speedRef.current;
  ...
  const wait = useCallback(async (ms: number, id: number) => {
    // Fixed tick: the remaining time must fall by a constant amount, otherwise a
    // speed below 1 halves it forever and the stage never completes.
    const TICK = 50;
    let remaining = ms;
    while (remaining > 0) {
      if (runId.current !== id) throw CANCELLED;
      await new Promise((r) => setTimeout(r, TICK));
      if (!pausedRef.current) remaining -= TICK * speedRef.current;
    }
    if (runId.current !== id) throw CANCELLED;
  }, []);
